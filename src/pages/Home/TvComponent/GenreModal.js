...
const GenreModal = ({ genreModal, onCloseGenreModal, onChangeGenre }) => {
  ...
  const buttonList = genreList.map((genre) => <button type="button" className="genreBtns" key={genre.id} onClick={() => onChangeGenre(genre.id)}>{genre.name.toUpperCase()}</button>);

  return (
    <div role="button" className={`genreModal ${genreModal && 'genreModalOpen'}`} onClick={onCloseGenreModal}>
