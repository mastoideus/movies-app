...
const TvComponent = () => {
  ...
  const openMovieModalHandler = (id) => {
    ...
    const movieInfo = copyData.find((movie) => movie.id === id);
  ...
  const dateButtonHandler = (value1, value2) => {
    ...
    const filteredDateArray = copyAllMovies.filter((movie) => movie.release_date >= value1 && movie.release_date <= value2);
  ...
  const moviesList = movieData.filter((movie) => {
    if (term === '') {
      return movie;
    } if (movie.title.split(' ')[0].toLowerCase().includes(term.toLowerCase())) {
      return movie;
    }
  }).map((movie) => (
    <MovieItem
